...
bacterial_infections <- micro_long_positives_only %>% 
  filter(specimen_group == "Blood") %>% 
    # 10/25/21 - encounter type is not longer included, so deleted from filter step. Will have to handle elsewhere.
  left_join(adt_long) %>% # We now have one row per micro result x adt record
  filter(collected_dt > adt_inpatient_dt, collected_dt < adt_discharge_dt,
  ...
  select(mrn, abnormal_result_logical, organism_name, organism_category, collected_dt, order_dt, adt_inpatient_dt, adt_discharge_dt, adt_visit) %>% 
  group_by(mrn, adt_visit, organism_name) %>% 
  arrange(mrn, adt_visit, organism_name, collected_dt, order_dt) %>% 
  mutate(collected_dt = as_datetime(collected_dt),
         previous_collected_dt = lag(collected_dt, n = 1, default = NA),
         time_since_prev = as.duration(collected_dt - previous_collected_dt),
         ...
         start_new_infection_dt = case_when(time_since_prev > as.duration("30 days") ~ collected_dt),
  ...
  summarize(first_dt = min(collected_dt, na.rm = T),
            last_dt = max(collected_dt, na.rm = T),
