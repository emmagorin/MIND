...
temp <- adt_long_patients %>% 
  ...
  mutate(previous_discharge_dt = lag(adt_discharge_dt, n = 1, default = NA), 
         ...
         covid_admission2 = !(adt_inpatient_dt >= start_non_covid_admissions_dt)) %>% 
              # covid_admission2 will be true if covid test occurs within -2 - 7 days of inpatient admission, or if they occur after this but with lapses of < 48 hours since a previous appointment. Ie, if someone keeps leaving and being re-admitted in < 48 hrs, all visits will be considered to be covid visits until they have been out of the hospital for at least 48 hrs.
  select(encounter_id, covid_admission2)
```

Merge adt_long_patients with temp. temp only adds covid_admission2 variable. 10/18/21
```{r}
adt_long_patients <- adt_long_patients %>% 
  left_join(temp)
```
...
This is the dataset I originally called "duration of bacteremia". It contains one observation per infection for bacterial infections from the micro dataset.
