...
add 7/5-7/6: Set up dataset of covid stays merged with bacterial infections
```{r}
...
adt_patients_bacterial_temp <- adt_long_patients %>% 
  filter(covid_admission == T) %>% # keep only "covid admissions"
...
#make adt_patients_bacterial_temp2 with one row per mrn:
adt_patients_bacterial_temp2 <- adt_patients_bacterial_temp %>% 
  group_by(mrn) %>% 
  summarize(first_inpatient_dt = min(adt_inpatient_dt),
            last_discharge_dt = max(adt_discharge_dt),
            bacterial_infection = as.logical(max(bacterial_infection))) %>% 
...
anova <- aov(covid_days_in_hospital ~ bacterial_infection, data = adt_patients_bacterial_temp2)
summary(anova)
...
yes_bacterial_infection <- adt_patients_bacterial_temp2 %>% filter(bacterial_infection == T)
no_bacterial_infection <- adt_patients_bacterial_temp2 %>% filter(bacterial_infection == F)
...
temp <- adt_patients_bacterial_temp2 %>% 
  left_join(patients) %>% 
