...
#ADT wide:
adt_wide_orig <- read_csv("./wide_files_2_24/adt_summary2021-02-24.csv")

#ADT long:
adt_long_orig <- read_csv("./long_files_2_24/adt_long2021-02-24.csv")

#Covid wide:
covid_wide_orig <- read_csv("./wide_files_2_24/covid_testing_wide2021-02-24.csv")

#Covid long:
covid_long_orig <- read_csv("./long_files_2_24/covid_pcr_long2021-02-24.csv")

#Micro long:
micro_long_orig <- read_csv("./long_files_2_24/micro_long2021-02-24.csv")
```
...
```
Based on:
any_pcr_pos: 11,352 individuals had covid ever
covid_pcr_result: 11,351
covid_pcr_result2: 11,843
It looks like the "any_pcr_pos" variable was created using the covid_pcr_result variable in the original covid_long dataset, rather than the covid_pcr_result2 variable. I think this excludes pretty much anyone who initially had an indeterminate result and then a positive one. This affects ~ 4% of people with pos covid tests.
...
micro_long <- micro_long_orig %>% 
  ...
  rename(admit_dt = admit_date_time,
         specimen_collected_dt = specimen_collected_date_time) %>% # put date time variables in consistent naming format.
...
Probably get rid of the following couple of things:

    ### Subset of columns of adt_wide data:

    
    ### Merge: adt_wide_covid_wide
    Merge wide covid data and select wide ADT data. Keep only individuals in adt dataset. 
    There are some covid tests that don't correspond to anyone in the adt dataset, this leaves those out.
    ```{r}
    #adt_wide_covid_wide <- left_join(adt_select, covid_wide, by = "mrn")
    ```
...
# no mrns appear in adt_wide and not adt_long
...
adt_long_not_wide %>% 
  summarize(min_dat = min(adt_event_date), 
            max_date = max(adt_event_date), 
            median_date = median(adt_event_date))
...
## adt_long_covid_wide_micro_long

      ```{r}
      adt_long_covid_wide_micro_long <- adt_long_covid_wide_micro_long %>% 
        janitor::clean_names() # makes cleaner names, standardizes them with snake formal and lower case. 
      ```
...
micro_long_positives_only <- micro_long %>% 
  ...
  #group_by(mrn) %>% 
  #summarize(first_abnormal_result = min(specimen_collected_dt))
```
...
micro_long %>% 
  ...
  mutate(bacterial_after_covid = 
           ifelse(covid_pos_dt <= specimen_collected_dt, T, F), 
...
micro_long_positives_only %>% 
  ...
  summarize(n = n()) %>% 
  arrange(desc(n)) %>% 
  mutate(org_spec = paste(organism_name, specimen_group, "_")) %>% 
  slice_head(n = 10) %>% 
  ggplot(aes(x = org_spec, y = n)) +
  geom_histogram()
...
micro_long_positives_only %>% 
  group_by(organism_name) %>% 
  arrange(desc())
  ggplot(aes(x = organism_category)) +
  geom_bar()



# Try this but make horizontal?
micro_long_positives_only
  group_by(organism_name, specimen_group) %>% 
...
```{r}
micro_long %>% 
  group_by(mrn, organism_name) %>% # one row per person-organism (maybe there need to be more? repeat infections with same organism??)
  mutate(abnormal_result_dt = case_when(abnormal_result == T ~ specimen_collected_dt),
         last_abnormal_result_dt = max(abnormal_result_dt),
         neg_result_after_pos ..............) %>% 
...
micro_long %>% 
  ...
  ggplot(aes(x = specimen_collected_dt)) +
  geom_histogram()
```
