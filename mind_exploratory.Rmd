...
for(i in c("STAPHYLOCOCCUS EPIDERMIDIS",
  ...
  n_non_covid <- nrow(duration_of_bacteremia %>% 
                        filter(organism_name == i,
                               covid_and_bacterial_same_admission == F))
  n_covid <- nrow(duration_of_bacteremia %>% 
                        filter(organism_name == i,
                               covid_and_bacterial_same_admission == T))
  
  non_covid_plot <- duration_of_bacteremia %>% 
    filter(organism_name == i,
           covid_and_bacterial_same_admission == F) %>% 
    mutate(days_bacteremia = duration_bacteremia/86400) %>% 
    ggplot(aes(x = days_bacteremia)) +
  ...
  covid_plot <- duration_of_bacteremia %>% 
    filter(organism_name == i,
           covid_and_bacterial_same_admission == T) %>% 
    mutate(days_bacteremia = duration_bacteremia/86400) %>% 
    ggplot(aes(x = days_bacteremia)) +
  ...
  plot_covid_non_covid <- duration_of_bacteremia %>% 
    filter(organism_name == i) %>% 
    mutate(days_bacteremia = duration_bacteremia/86400) %>% 
