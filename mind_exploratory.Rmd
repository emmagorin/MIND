...
duration_of_bacteremia <- micro_long_positives_only %>% 
  ...
  mutate(previous_order_dt = lag(order_dt, n = 1, default = NA),
         time_since_prev_ord = as.duration(order_dt - previous_order_dt),
         max_lapse_in_visit = as.duration(max(time_since_prev_ord, na.rm = T)),
         max_lapse_in_visit = replace(max_lapse_in_visit, max_lapse_in_visit == -Inf, 0)) %>% 
    # produces some -Infs, but that shouldn't cause a problem.
