...
duration_of_bacteremia <- micro_long_positives_only %>% 
  ...
  mutate(previous_order_dt = lag(order_dt, n = 1, default = NA),
         ...
         infection_number = case_when(is.na(start_new_infection_dt)==T ~ 1, # infection_number is 1 if there was never a lapse of >30 days
                                      is.na(start_new_infection_dt)==F & order_dt<start_new_infection_dt ~1, # infection_number is 1 if there was a lapse of 30 days and this test took place before lapse
                                      is.na(start_new_infection_dt)==F & order_dt>=start_new_infection_dt ~ 2)) %>% # infection_number is 2 if there was a lapse of 30 days and test took place after lapse.
  ...
  left_join(patients) %>% 
  select(mrn, covid_pos_ever, covid_pos_dt, organism_name, first_dt, last_dt, max_lapse_in_visit, duration_bacteremia)
...
duration_of_bacteremia <- duration_of_bacteremia %>% 
  ungroup() %>% 
