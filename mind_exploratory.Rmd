...
covid_admissions_bacterial2 <- covid_admissions_bacterial %>% 
  group_by(mrn) %>% 
  summarize(first_inpatient_dt = min(adt_inpatient_dt),
            last_discharge_dt = max(adt_discharge_dt),
            bacterial_infection = as.logical(max(bacterial_infection))) %>% 
...
(In other words: What happens if we consider re-admissions that aren't within the timeframe (48 hrs before covid diagnosis through 7 days after) that would label them a "covid admission", but do occur shortly after a covid discharge, suggesting that they're likely still related to the same health concerns?)
...
Now, set up new rule and check.
This is a little tricky - want to assign new covid_admission2 variable based on time since previous admission, but depending on whether previous admission was a covid admission - but, this should be recursive, so even if someone has had 5 admissions or whatever, if there's little time in between and the first one in the series was a covid admission, it should be considered a covid admission.

Try setting this up similarly to how I set up "bacterial_infection" dataframe in the mind_setup file.
```{r}
temp <- adt_long_patients %>% 
  group_by(mrn) %>% 
  mutate(covid_first_admission_ever = max (covid_admission)) %>% 
    # covid_first_admission is an indicator of whether patient ever had an admission considered a covid admission by our prvious criteria.
  ...
  mutate(previous_discharge_dt = lag(adt_discharge_dt, n = 1, default = NA), 
         ...
         start_non_covid_admissions_dt = case_when(time_since_prev_discharge > as.duration("2 days") ~ adt_inpatient_dt),
            # start date of admissions that are not considered to be covid admissions, because > 2 days have elapsed
         ...
         covid_admission2 = !(adt_inpatient_dt >= start_non_covid_admissions_dt))
              # covid_admission2 will be true if covid test occurs within -2 - 7 days of inpatient admission, or if they occur after this but
              # with lapses of < 48 hours since a previous appointment. Ie, if someone keeps leaving and being re-admitted in < 48 hrs, all
              # visits will be considered to be covid visits until they have been out of the hospital for at least 48 hrs. 
```
...
Notes 10/15: I thought I finally had this ascertainment right, but it doesn't make sense that there should be more ppl with one covid admission - this should go down. (in the summary above). I think what may be going on is that we filtered for ppl who were covid pos ever, not who had covid INPATIENT ADMISSIONS ever - so inpatient visits captured without a 48 hr lapse but later than the original window (no "first" covid admission) still get captured in covid_admission2. We need to make sure that each person at least initially has a covid admission. 
try 48, 24, 72, compare, then run by AC and Jason
