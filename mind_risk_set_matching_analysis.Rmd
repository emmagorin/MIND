...
matching_df <- matching_df_all_vars %>% 
  ...
  mutate(age_at_admission = replace_na(age_at_admission, 200),
    ...
    # ok I realize this is not great, but I'm going to plug in values for the few missing things here just to see if I can get the matching algorithm to work. 
    # for now, include these variables only.
```

Run matching procedure:
```{r}
pairs <- coxpsmatch(n_pairs = 1, data = matching_df, id = "mrn", time = "time", trt_time = "trt_time")

                    covariates = c("age_at_admission", "sex"))
                    
                    
                    , "combined_race_ethnicity", "zipcode", "insurance_type", "adt_site", "adt_care_level"))
```
