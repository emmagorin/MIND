...
```
Ok, weirdly is.na(last_discharge_dt) is false, but there clearly are some where this is missing, based on the next check above. For now, let's just drop these.
```{r}
matching_df <- matching_df_all_vars %>% filter(last_discharge_dt > first_inpatient_dt)
```
...
#### Set up dataframe with just time-invarying covars
```{r}
...
```{r}
coxph_df <- tmerge(data1 = matching_df, data2 = matching_df, tstart = first_inpatient_dt, tstop = last_discharge_dt, id = mrn)

coxph_df <- tmerge(coxph_df, icu_stay, id = mrn, tdc = icu)
