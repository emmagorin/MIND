...
matching_df_all_vars <- df %>% 
  ...
  mutate(first_inpatient_dt = min(adt_inpatient_dt),
         ...
         first_path_of_interest_dt = min(path_of_interest_dt),
         prior_infection_at_site = ifelse(is.na(first_path_of_interest_dt), NA, (first_bacterial_start_dt < first_path_of_interest_dt)),

# Note 1/5: The above is interesting to explore, and should show us whether ppl who have the pathogen have previously had another infection. However, can't really include this in this way in the coxph model - what would be the time threshold for the controls? I think I'll have to incorporate this as a time-varying covariate....


         trt_time = case_when(!(first_bacterial_start_dt == Inf) ~ as.numeric(((first_bacterial_start_dt) - (first_inpatient_dt)) / ddays(1)))) %>% 
  
  group_by(mrn) %>% arrange(mrn, first_dt) %>% mutate(index = row_number()) %>% 
  # Note 1/5: adjust above line so that we're keeping the appropriate line (first instance of given pathogen)
...
path_of_interest <- df %>% 
  ...
  mutate(first_inpatient_dt = min(adt_inpatient_dt),
         ...
         path_of_interest_dt = ifelse(organism_name == path, first_dt, NA),
         first_path_of_interest_dt = min(path_of_interest_dt),
         time = case_when(!(first_oath_of_interest_dt == Inf) ~ as.numeric(((first_oath_of_interest_dt) - (first_inpatient_dt)) / ddays(1)))) %>% # surely can make it so this doesn't have infs. 
  # somehow filter so for ppl who have path of interest, other pathogens aren't included........... or mauybe don't need to ???
  ...
  # add select to minimize unnecessary columns
