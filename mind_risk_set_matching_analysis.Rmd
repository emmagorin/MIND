...
matches <- matching_df_all_vars %>% 
  ...
  arrange(trt_time) %>% 
  mutate(covid_days_in_hospital = NA, ctrl_mrn = NA,  ctrl_covid_days_in_hospital = NA)
```
...
for(i in 1:nrow(matches)) {
  ...
  # plug info about infected person and control into matches df
  matches$covid_days_in_hospital[i] <- infected_person$covid_days_in_hospital[1]
  matches$ctrl_mrn[i] <- control$mrn[1]
  matches$ctrl_covid_days_in_hospital[i] <- control$covid_days_in_hospital
}

rm(infection_time, infected_person, infected_propensity, riskset, control)
```
...
Note 8/25: not sure why I even needed to do this:
```{r}
...
Generate basic stats on infections:
```{r}
n_infections <- nrow(matches)

#site <- "urine"

median_infection_time <- median(matches$trt_time)
iqr_infection_time <- IQR(matches$trt_time)
infection_time_quantiles <- quantile(matches$trt_time, c(.25, .5, .75))

timing_infections <-  matches %>% 
  ggplot() +
  geom_density(aes(x = trt_time), alpha = .5, fill = "light blue") +
  labs(title = paste("Time of infection detection", "-", site, "-", pathogen), 
...
ttest_output <- tibble(sample = c("unmatched", "matched"), 
                       estimated_difference_LOS = c(0, 0),
                       CI_lb = c(0, 0),
                       CI_ub = c(0,0))


# make subsets of data to use for t-tests (there's surely a quicker way, but this should work)
infected <- matches %>% select(mrn) %>% left_join(matching_df_all_vars) 
all_uninfected <- matching_df_all_vars %>% filter(status == F)
controls <- matches %>% select(ctrl_mrn) %>% mutate(mrn = ctrl_mrn) %>% left_join(matching_df_all_vars)
  
# I wonder how many unique controls we used:
n_unique_controls <- n_distinct(controls$mrn)

# what proportion of controls were people with subsequent infections:
n_infected_controls <- controls %>% ungroup() %>% summarize(sum = sum(status))
n_infected_controls <- n_infected_controls$sum[1]
prop_infected_controls <- round(n_infected_controls/n_infections, 3)
...
# KS test results for matched sample:
ks_matched <- ks.test(matches$covid_days_in_hospital, matches$ctrl_covid_days_in_hospital)
ks_matched_d <- round(ks_matched$statistic, 3)
ks_matched_p <- round(ks_matched$p.value, 3)
# KS test results for unmatched sample:
ks_unmatched <- ks.test(infected$covid_days_in_hospital, all_uninfected$covid_days_in_hospital)
ks_unmatched_d <- round(ks_unmatched$statistic, 3)
ks_unmatched_p <- round(ks_unmatched$p.value, 3)
...
```{r}
#histogram:
matched_histogram <- matches %>% 
  #set up in easier format for matching. Not perfectly tidy, but will work for this.
  rename(control = ctrl_covid_days_in_hospital, infected = covid_days_in_hospital) %>% 
  pivot_longer(cols = c(control, infected), names_to = "infect_control", values_to = "days_in_hospital") %>% 
  ggplot() +
  geom_histogram(aes(x = days_in_hospital, group = infect_control, fill = infect_control), alpha = .5, binwidth = 1, position = "identity") +
  labs(title = paste(plot1title, "-", pathogen), 
       x = "Length of stay (days)",
       fill = "LOS") +
  theme(legend.position = c(.9,.9))

#density plot:
matched_density <- matches %>% 
  #set up in easier format for matching. Not perfectly tidy, but will work for this.
  rename(control = ctrl_covid_days_in_hospital, infected = covid_days_in_hospital) %>% 
  pivot_longer(cols = c(control, infected), names_to = "infect_control", values_to = "days_in_hospital") %>% 
  ggplot() +
  geom_density(aes(x = days_in_hospital, group = infect_control, fill = infect_control), alpha = .5) +
  labs(title = paste(plot1title, "-", pathogen), 
       x = "Length of stay (days)",
       fill = "LOS") +
  theme(legend.position = c(.9,.9))
...
infected_quantiles <- round(quantile(matches$covid_days_in_hospital, c(.25,.5,.75)), 2)
control_quantiles <- round(quantile(matches$ctrl_covid_days_in_hospital, c(.25,.5,.75)), 2)

matched_dots <- matches %>% 
   #set up in easier format for matching. Not perfectly tidy, but will work for this.
  rename(control = ctrl_covid_days_in_hospital, infected = covid_days_in_hospital) %>% 
  pivot_longer(cols = c(control, infected), names_to = "infect_control", values_to = "days_in_hospital") %>% 
  mutate(infect_control = factor(infect_control, levels = c("infected", "control"))) %>% 
  ggplot(aes(x = infect_control, y = days_in_hospital)) +
  ggbeeswarm::geom_quasirandom(
    shape = 21, color = "white", 
    alpha = 0.3, size = 3,
    aes(fill = infect_control)
  ) +
  #scale_fill_manual(values = brewer.pal(8, "Accent")[1:2]) +
  labs(x = "",
       y = "Days in hospital",
       title = matched_plot_title, 
       caption = paste("Infected 25, 50 and 75 percentiles:", infected_quantiles[1], ",", infected_quantiles[2], ",", infected_quantiles[3], "\nControl 25, 50 and 75 percentiles:", control_quantiles[1], ",", control_quantiles[2], ",", control_quantiles[3], "\nKS D statistic:", ks_matched_d, "KS p-value:", ks_matched_p)) + # to fill in with t-test results, etc. 
  theme_classic() +
  theme(
...
#etc. could make a box plot or violin plot or something
```

Plot comparison pre-matching:
To do- - add N for infected and not infected
```{r}
#make quantiles
uninfected_only <- matching_df_all_vars %>% filter(status == F)
uninfected_quantiles <- round(quantile(uninfected_only$covid_days_in_hospital, c(.25,.5,.75)), 2)

# dot plot:
unmatched_plot_title <- paste("LOS unmatched - ", site, ", ", pathogen, sep = "")
unmatched_dots <- matching_df_all_vars %>% 
   #set up in easier format for matching. Not perfectly tidy, but will work for this.
  mutate(infect_control = ifelse(status == T, "infected", "not infected")) %>% 
  ggplot(aes(x = infect_control, y = covid_days_in_hospital)) +
  ggbeeswarm::geom_quasirandom(
    shape = 21, color = "white", 
    alpha = 0.3, size = 3,
    aes(fill = infect_control)
  ...
  labs(x = "",
       y = "Days in hospital",
       title = unmatched_plot_title, 
       caption = paste("Infected 25, 50 and 75 percentiles:", infected_quantiles[1], ",", infected_quantiles[2], ",", infected_quantiles[3], "\nControl 25, 50 and 75 percentiles:", uninfected_quantiles[1], ",", uninfected_quantiles[2], ",", uninfected_quantiles[3], "\nKS D statistic:", ks_unmatched_d, "KS p-value:", ks_unmatched_p)) +
  theme_classic() +
  theme(
    text = element_text(size = 12, face = "bold", color = "black"),
    axis.text = element_text(color = "black"),
...
Print results:
```{r}
print(paste("Pathogen(s):", pathogen, "; Site:", site))
print(paste("Number of infections among covid patients: ", n_infections))

timing_infections

coxph_output

print(paste("Controls are now selected with replacement. Number of unique individuals selected as controls: ", n_unique_controls))
print(paste("Controls are selected only based on information available at time of infection. Proportion of controls who subsequently were infected with this pathogen at this site: ", prop_infected_controls))

kable(ttest_output)

wrap_plots(
  unmatched_dots, matched_dots, nrow = 1
)
```
