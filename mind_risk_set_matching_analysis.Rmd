...
matching_df_all_vars <- df %>% 
  ...
  mutate(first_inpatient_dt = min(adt_inpatient_dt),
         last_discharge_dt = max(adt_discharge_dt, na.rm = T),
...
Check: does anyone have a missing first_inpatient_dt or last_discharge_dt? Is inpatient_dt < last_discharge_dt for anyone?
```{r}
...
### Set up dataframe for coxph - time-invarying only
...
coxph_df <- matching_df_all_vars %>%
  ...
  select(mrn, first_bacterial_start_dt, last_discharge_dt, time_dt, time)
...
```{r}
icu_stay <- df %>% 
  mutate(icu = (adt_care_level == "Intensive Care (ICU)"),
         start = adt_event_date,
         stop = adt_discharge_date) %>% 
  select(mrn, adt_care_level, icu, start, stop)
...
```{r}
# set up dataframe
coxph_df2 <- tmerge(data1 = coxph_df, data2 = coxph_df, tstart = first_inpatient_dt, tstop = last_discharge_dt, id = mrn, infection = event(time, status))
# What if I do it without the tstart and tstop:
coxph_df2 <- tmerge(data1 = coxph_df, data2 = coxph_df, id = mrn, infection = event(time, status))
# now this works - but the time is not in a time format.

# What if I use absolute times for the event and for tstart and tstop:
coxph_df2 <- tmerge(data1 = coxph_df, data2 = coxph_df, tstart = first_inpatient_dt, tstop = last_discharge_dt, id = mrn, infection = event(time_dt, status))

# Add ICU variable
coxph_df <- tmerge(coxph_df, icu_stay, id = mrn, icu = tdc(icu))


```
8/11/22 notes: problem - the infection event that I made is coming up as "false" for everything. maybe because the time doesn't fall into the tstart/tstop range? Possible solutions: define "time" differently so that it's an actualy time; or get rid of the start and stop
