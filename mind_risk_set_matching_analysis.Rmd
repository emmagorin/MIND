...
coxph_df <- matching_df_all_vars %>% 
  ...
  filter(any(is.na(age_at_admission), is.na(sex), is.na(adt_status), is.na(covid_visits_start_date), is.na(combined_race_ethnicity), is.na(insurance_type), is.na(time), is.na(status)) == F) 
...
infected <- coxph_df %>% filter(status == T)
all_uninfected <- coxph_df %>% filter(status == F)
controls <- matched_ppl_df %>% filter(status == F)

# unmatched sample:
coxph_df %>% 
  group_by(status) %>% 
  summarize(mean_days_in_hosp = mean(covid_days_in_hospital),
            sd_days_in_hosp = sd(covid_days_in_hospital))
...
t.test(x = infected$covid_days_in_hospital, y = controls$covid_days_in_hospital)
