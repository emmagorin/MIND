...
step_2 <- df %>% 
  ...
  mutate(time = 0,
         other_infection = 0)
# is this step necessary? It's a question.....
...
step_3 <- df %>% 
  group_by(mrn) %>% 
  mutate(other_infection_dt = ifelse((organism_name!=path & !is.na(organism_name)), as_datetime(first_dt), NA),
         other_infection_dt = as_datetime(other_infection_dt))

# ok, this is a good start: to make it fit with Jeff's suggestion, will need to add rows for 5 days after infection, basically, then merge all with step 3. These will be categorical variables that are analyzed as separate vars.....
